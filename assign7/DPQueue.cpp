...
namespace CS3358_SSII2019_A7
{
   ...
   p_queue::value_type p_queue::front() const		//2nd Stage
   {
	assert(size() > 0);
	return heap[0].data;
   }
   ...
   bool p_queue::is_leaf(size_type i) const	//2nd stage
   // Pre:  (i < used)
   ...
   p_queue::size_type
   p_queue::big_child_index(size_type i) const		//2nd stage
   ...
   //       than that of the other child, if there is one.)
   {
	...
	size_type iLeft = (i * 2) + 1;
	size_type iRight = (i * 2) + 2;
	...
	if(iRight < used && heap[iRight].priority > heap[iLeft].priority)
		return iRight;
	else
		return iLeft;
   }
   ...
   p_queue::size_type
   p_queue::big_child_priority(size_type i) const	//2nd stage
   // Pre:  is_leaf(i) returns false
